---
name: write-diataxis-article-reference-review
description: Reviews articles against the Diátaxis Reference Review rubric from jeffbailey.us. Direct shortcut — skips framework detection. Use when the user says /write:diataxis-article-reference-review.
---

# Diátaxis Reference Review

Direct shortcut to review articles against the Diátaxis Reference Review rubric. Skips framework detection — use `/write:review` if you want automatic detection instead.
...
1. Check if `~/.claude/cache/writing-prompts/diataxis-article-reference-review.md` exists
2. If it exists, read and use it
3. If it does not exist, fetch and cache it:
   ```bash
   mkdir -p ~/.claude/cache/writing-prompts && curl -s "https://jeffbailey.us/prompts/diataxis-article-reference-review/raw.html" | pandoc -f html -t markdown --wrap=none -o ~/.claude/cache/writing-prompts/diataxis-article-reference-review.md
   ```
...
2. **Load the review prompt** -- Load the Diátaxis Reference Review prompt (slug: `diataxis-article-reference-review`) using the caching steps above.
