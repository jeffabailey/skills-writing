---
name: write-problem-agitate-solve-article-review
description: Reviews articles against the Problem-Agitate-Solve Article Review rubric from jeffbailey.us. Direct shortcut — skips framework detection. Use when the user says /write:problem-agitate-solve-article-review.
---

# Problem-Agitate-Solve Article Review

Direct shortcut to review articles against the Problem-Agitate-Solve Article Review rubric. Skips framework detection — use `/write:review` if you want automatic detection instead.
...
1. Check if `~/.claude/cache/writing-prompts/problem-agitate-solve-article-review.md` exists
2. If it exists, read and use it
3. If it does not exist, fetch and cache it:
   ```bash
   mkdir -p ~/.claude/cache/writing-prompts && curl -s "https://jeffbailey.us/prompts/problem-agitate-solve-article-review/raw.html" | pandoc -f html -t markdown --wrap=none -o ~/.claude/cache/writing-prompts/problem-agitate-solve-article-review.md
   ```
...
2. **Load the review prompt** -- Load the Problem-Agitate-Solve Article Review prompt (slug: `problem-agitate-solve-article-review`) using the caching steps above.
