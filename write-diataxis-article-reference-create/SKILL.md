---
name: write-diataxis-article-reference-create
description: Creates lookup-oriented reference articles using the Diátaxis Reference Create prompt from jeffbailey.us. Direct shortcut — skips framework selection. Use when the user says /write:diataxis-article-reference-create.
---

# Diátaxis Reference Create

Direct shortcut to create articles using the Diátaxis Reference Create framework. Skips framework selection — use `/write:article` if you want recommendations instead.
...
1. Check if `~/.claude/cache/writing-prompts/diataxis-article-reference-create.md` exists
2. If it exists, read and use it
3. If it does not exist, fetch and cache it:
   ```bash
   mkdir -p ~/.claude/cache/writing-prompts && curl -s "https://jeffbailey.us/prompts/diataxis-article-reference-create/raw.html" | pandoc -f html -t markdown --wrap=none -o ~/.claude/cache/writing-prompts/diataxis-article-reference-create.md
   ```
...
2. **Load the prompt** -- Load the Diátaxis Reference Create prompt (slug: `diataxis-article-reference-create`) using the caching steps above.
