---
name: write-diataxis-article-tutorials-create
description: Creates learning-by-doing tutorial articles using the Diátaxis Tutorials Create prompt from jeffbailey.us. Direct shortcut — skips framework selection. Use when the user says /write:diataxis-article-tutorials-create.
---

# Diátaxis Tutorials Create

Direct shortcut to create articles using the Diátaxis Tutorials Create framework. Skips framework selection — use `/write:article` if you want recommendations instead.
...
1. Check if `~/.claude/cache/writing-prompts/diataxis-article-tutorials-create.md` exists
2. If it exists, read and use it
3. If it does not exist, fetch and cache it:
   ```bash
   mkdir -p ~/.claude/cache/writing-prompts && curl -s "https://jeffbailey.us/prompts/diataxis-article-tutorials-create/raw.html" | pandoc -f html -t markdown --wrap=none -o ~/.claude/cache/writing-prompts/diataxis-article-tutorials-create.md
   ```
...
2. **Load the prompt** -- Load the Diátaxis Tutorials Create prompt (slug: `diataxis-article-tutorials-create`) using the caching steps above.
