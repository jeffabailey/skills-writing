---
name: write-lesson-planning-article-create
description: Creates multi-framework lesson planning articles using the Lesson Planning Article Create prompt from jeffbailey.us. Direct shortcut — skips framework selection. Use when the user says /write:lesson-planning-article-create.
---

# Lesson Planning Article Create

Direct shortcut to create articles using the Lesson Planning Article Create framework. Skips framework selection — use `/write:article` if you want recommendations instead.
...
1. Check if `~/.claude/cache/writing-prompts/lesson-planning-article-create.md` exists
2. If it exists, read and use it
3. If it does not exist, fetch and cache it:
   ```bash
   mkdir -p ~/.claude/cache/writing-prompts && curl -s "https://jeffbailey.us/prompts/lesson-planning-article-create/raw.html" | pandoc -f html -t markdown --wrap=none -o ~/.claude/cache/writing-prompts/lesson-planning-article-create.md
   ```
...
2. **Load the prompt** -- Load the Lesson Planning Article Create prompt (slug: `lesson-planning-article-create`) using the caching steps above.
