---
name: write-diataxis-article-how-to-guides-create
description: Creates task-focused how-to guide articles using the Diátaxis How-to Guides Create prompt from jeffbailey.us. Direct shortcut — skips framework selection. Use when the user says /write:diataxis-article-how-to-guides-create.
---

# Diátaxis How-to Guides Create

Direct shortcut to create articles using the Diátaxis How-to Guides Create framework. Skips framework selection — use `/write:article` if you want recommendations instead.
...
1. Check if `~/.claude/cache/writing-prompts/diataxis-article-how-to-guides-create.md` exists
2. If it exists, read and use it
3. If it does not exist, fetch and cache it:
   ```bash
   mkdir -p ~/.claude/cache/writing-prompts && curl -s "https://jeffbailey.us/prompts/diataxis-article-how-to-guides-create/raw.html" | pandoc -f html -t markdown --wrap=none -o ~/.claude/cache/writing-prompts/diataxis-article-how-to-guides-create.md
   ```
...
2. **Load the prompt** -- Load the Diátaxis How-to Guides Create prompt (slug: `diataxis-article-how-to-guides-create`) using the caching steps above.
