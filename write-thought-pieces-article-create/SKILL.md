---
name: write-thought-pieces-article-create
description: Creates exploratory thought piece articles using the Thought Pieces Article Create prompt from jeffbailey.us. Direct shortcut — skips framework selection. Use when the user says /write:thought-pieces-article-create.
---

# Thought Pieces Article Create

Direct shortcut to create articles using the Thought Pieces Article Create framework. Skips framework selection — use `/write:article` if you want recommendations instead.
...
1. Check if `~/.claude/cache/writing-prompts/thought-pieces-article-create.md` exists
2. If it exists, read and use it
3. If it does not exist, fetch and cache it:
   ```bash
   mkdir -p ~/.claude/cache/writing-prompts && curl -s "https://jeffbailey.us/prompts/thought-pieces-article-create/raw.html" | pandoc -f html -t markdown --wrap=none -o ~/.claude/cache/writing-prompts/thought-pieces-article-create.md
   ```
...
2. **Load the prompt** -- Load the Thought Pieces Article Create prompt (slug: `thought-pieces-article-create`) using the caching steps above.
