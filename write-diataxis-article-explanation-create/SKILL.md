---
name: write-diataxis-article-explanation-create
description: Creates conceptual explanation articles using the Diátaxis Explanation Create prompt from jeffbailey.us. Direct shortcut — skips framework selection. Use when the user says /write:diataxis-article-explanation-create.
---

# Diátaxis Explanation Create

Direct shortcut to create articles using the Diátaxis Explanation Create framework. Skips framework selection — use `/write:article` if you want recommendations instead.
...
1. Check if `~/.claude/cache/writing-prompts/diataxis-article-explanation-create.md` exists
2. If it exists, read and use it
3. If it does not exist, fetch and cache it:
   ```bash
   mkdir -p ~/.claude/cache/writing-prompts && curl -s "https://jeffbailey.us/prompts/diataxis-article-explanation-create/raw.html" | pandoc -f html -t markdown --wrap=none -o ~/.claude/cache/writing-prompts/diataxis-article-explanation-create.md
   ```
...
2. **Load the prompt** -- Load the Diátaxis Explanation Create prompt (slug: `diataxis-article-explanation-create`) using the caching steps above.
