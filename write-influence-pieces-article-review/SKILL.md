---
name: write-influence-pieces-article-review
description: Reviews articles against the Influence Pieces Article Review rubric from jeffbailey.us. Direct shortcut — skips framework detection. Use when the user says /write:influence-pieces-article-review.
---

# Influence Pieces Article Review
...
1. Check if `~/.claude/cache/writing-prompts/influence-pieces-article-review.md` exists
2. If it exists, read and use it
3. If it does not exist, fetch and cache it:
   ```bash
   mkdir -p ~/.claude/cache/writing-prompts && curl -s "https://jeffbailey.us/prompts/influence-pieces-article-review/raw.html" | pandoc -f html -t markdown --wrap=none -o ~/.claude/cache/writing-prompts/influence-pieces-article-review.md
   ```
...
2. **Load the review prompt** -- Load the Influence Pieces Article Review prompt (slug: `influence-pieces-article-review`) using the caching steps above.
