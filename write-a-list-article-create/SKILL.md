---
name: write-a-list-article-create
description: Creates curated list articles using the A-List Article Create prompt from jeffbailey.us. Direct shortcut — skips framework selection. Use when the user says /write:a-list-article-create.
---

# A-List Article Create

Direct shortcut to create articles using the A-List Article Create framework. Skips framework selection — use `/write:article` if you want recommendations instead.
...
1. Check if `~/.claude/cache/writing-prompts/a-list-article-create.md` exists
2. If it exists, read and use it
3. If it does not exist, fetch and cache it:
   ```bash
   mkdir -p ~/.claude/cache/writing-prompts && curl -s "https://jeffbailey.us/prompts/a-list-article-create/raw.html" | pandoc -f html -t markdown --wrap=none -o ~/.claude/cache/writing-prompts/a-list-article-create.md
   ```
...
2. **Load the prompt** -- Load the A-List Article Create prompt (slug: `a-list-article-create`) using the caching steps above.
