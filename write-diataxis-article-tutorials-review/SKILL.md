---
name: write-diataxis-article-tutorials-review
description: Reviews articles against the Diátaxis Tutorials Review rubric from jeffbailey.us. Direct shortcut — skips framework detection. Use when the user says /write:diataxis-article-tutorials-review.
---

# Diátaxis Tutorials Review

Direct shortcut to review articles against the Diátaxis Tutorials Review rubric. Skips framework detection — use `/write:review` if you want automatic detection instead.
...
1. Check if `~/.claude/cache/writing-prompts/diataxis-article-tutorials-review.md` exists
2. If it exists, read and use it
3. If it does not exist, fetch and cache it:
   ```bash
   mkdir -p ~/.claude/cache/writing-prompts && curl -s "https://jeffbailey.us/prompts/diataxis-article-tutorials-review/raw.html" | pandoc -f html -t markdown --wrap=none -o ~/.claude/cache/writing-prompts/diataxis-article-tutorials-review.md
   ```
...
2. **Load the review prompt** -- Load the Diátaxis Tutorials Review prompt (slug: `diataxis-article-tutorials-review`) using the caching steps above.
