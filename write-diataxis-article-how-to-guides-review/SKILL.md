---
name: write-diataxis-article-how-to-guides-review
description: Reviews articles against the Diátaxis How-to Guides Review rubric from jeffbailey.us. Direct shortcut — skips framework detection. Use when the user says /write:diataxis-article-how-to-guides-review.
---

# Diátaxis How-to Guides Review

Direct shortcut to review articles against the Diátaxis How-to Guides Review rubric. Skips framework detection — use `/write:review` if you want automatic detection instead.
...
1. Check if `~/.claude/cache/writing-prompts/diataxis-article-how-to-guides-review.md` exists
2. If it exists, read and use it
3. If it does not exist, fetch and cache it:
   ```bash
   mkdir -p ~/.claude/cache/writing-prompts && curl -s "https://jeffbailey.us/prompts/diataxis-article-how-to-guides-review/raw.html" | pandoc -f html -t markdown --wrap=none -o ~/.claude/cache/writing-prompts/diataxis-article-how-to-guides-review.md
   ```
...
2. **Load the review prompt** -- Load the Diátaxis How-to Guides Review prompt (slug: `diataxis-article-how-to-guides-review`) using the caching steps above.
