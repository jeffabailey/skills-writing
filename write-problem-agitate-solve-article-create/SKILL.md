---
name: write-problem-agitate-solve-article-create
description: Creates urgency-driven problem-agitate-solve articles using the Problem-Agitate-Solve Article Create prompt from jeffbailey.us. Direct shortcut — skips framework selection. Use when the user says /write:problem-agitate-solve-article-create.
---

# Problem-Agitate-Solve Article Create

Direct shortcut to create articles using the Problem-Agitate-Solve Article Create framework. Skips framework selection — use `/write:article` if you want recommendations instead.
...
1. Check if `~/.claude/cache/writing-prompts/problem-agitate-solve-article-create.md` exists
2. If it exists, read and use it
3. If it does not exist, fetch and cache it:
   ```bash
   mkdir -p ~/.claude/cache/writing-prompts && curl -s "https://jeffbailey.us/prompts/problem-agitate-solve-article-create/raw.html" | pandoc -f html -t markdown --wrap=none -o ~/.claude/cache/writing-prompts/problem-agitate-solve-article-create.md
   ```
...
2. **Load the prompt** -- Load the Problem-Agitate-Solve Article Create prompt (slug: `problem-agitate-solve-article-create`) using the caching steps above.
