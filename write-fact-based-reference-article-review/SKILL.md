---
name: write-fact-based-reference-article-review
description: Reviews articles against the Fact-Based Reference Article Review rubric from jeffbailey.us. Direct shortcut — skips framework detection. Use when the user says /write:fact-based-reference-article-review.
---

# Fact-Based Reference Article Review

Direct shortcut to review articles against the Fact-Based Reference Article Review rubric. Skips framework detection — use `/write:review` if you want automatic detection instead.
...
1. Check if `~/.claude/cache/writing-prompts/fact-based-reference-article-review.md` exists
2. If it exists, read and use it
3. If it does not exist, fetch and cache it:
   ```bash
   mkdir -p ~/.claude/cache/writing-prompts && curl -s "https://jeffbailey.us/prompts/fact-based-reference-article-review/raw.html" | pandoc -f html -t markdown --wrap=none -o ~/.claude/cache/writing-prompts/fact-based-reference-article-review.md
   ```
...
2. **Load the review prompt** -- Load the Fact-Based Reference Article Review prompt (slug: `fact-based-reference-article-review`) using the caching steps above.
