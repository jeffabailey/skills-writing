---
name: write-tea-article-create
description: Creates topic-evidence-analysis articles using the TEA Article Create prompt from jeffbailey.us. Direct shortcut — skips framework selection. Use when the user says /write:tea-article-create.
---

# TEA Article Create

Direct shortcut to create articles using the TEA Article Create framework. Skips framework selection — use `/write:article` if you want recommendations instead.
...
1. Check if `~/.claude/cache/writing-prompts/tea-article-create.md` exists
2. If it exists, read and use it
3. If it does not exist, fetch and cache it:
   ```bash
   mkdir -p ~/.claude/cache/writing-prompts && curl -s "https://jeffbailey.us/prompts/tea-article-create/raw.html" | pandoc -f html -t markdown --wrap=none -o ~/.claude/cache/writing-prompts/tea-article-create.md
   ```
...
2. **Load the prompt** -- Load the TEA Article Create prompt (slug: `tea-article-create`) using the caching steps above.
