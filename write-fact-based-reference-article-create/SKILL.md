---
name: write-fact-based-reference-article-create
description: Creates authoritative fact-based reference articles using the Fact-Based Reference Article Create prompt from jeffbailey.us. Direct shortcut — skips framework selection. Use when the user says /write:fact-based-reference-article-create.
---

# Fact-Based Reference Article Create

Direct shortcut to create articles using the Fact-Based Reference Article Create framework. Skips framework selection — use `/write:article` if you want recommendations instead.
...
1. Check if `~/.claude/cache/writing-prompts/fact-based-reference-article-create.md` exists
2. If it exists, read and use it
3. If it does not exist, fetch and cache it:
   ```bash
   mkdir -p ~/.claude/cache/writing-prompts && curl -s "https://jeffbailey.us/prompts/fact-based-reference-article-create/raw.html" | pandoc -f html -t markdown --wrap=none -o ~/.claude/cache/writing-prompts/fact-based-reference-article-create.md
   ```
...
2. **Load the prompt** -- Load the Fact-Based Reference Article Create prompt (slug: `fact-based-reference-article-create`) using the caching steps above.
