---
name: write-backward-design-article-review
description: Reviews articles against the Backward Design Article Review rubric from jeffbailey.us. Direct shortcut — skips framework detection. Use when the user says /write:backward-design-article-review.
---

# Backward Design Article Review

Direct shortcut to review articles against the Backward Design Article Review rubric. Skips framework detection — use `/write:review` if you want automatic detection instead.
...
1. Check if `~/.claude/cache/writing-prompts/backward-design-article-review.md` exists
2. If it exists, read and use it
3. If it does not exist, fetch and cache it:
   ```bash
   mkdir -p ~/.claude/cache/writing-prompts && curl -s "https://jeffbailey.us/prompts/backward-design-article-review/raw.html" | pandoc -f html -t markdown --wrap=none -o ~/.claude/cache/writing-prompts/backward-design-article-review.md
   ```
...
2. **Load the review prompt** -- Load the Backward Design Article Review prompt (slug: `backward-design-article-review`) using the caching steps above.
